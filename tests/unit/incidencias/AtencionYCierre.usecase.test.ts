...
describe("CerrarCasoUseCase", () => {
  ...
  it("[negativo] lanza BusinessRuleError si el incidente no está en estado cerrable", async () => {
    ...
    await expect(new CerrarCasoUseCase(repo).execute("inc-7")).rejects.toThrow(BusinessRuleError);
  });

  it("[negativo] lanza NotFoundError cuando el incidente no existe", async () => {
    const repo = makeRepo();
    await expect(new CerrarCasoUseCase(repo).execute("no-existe")).rejects.toThrow(NotFoundError);
  });
});
