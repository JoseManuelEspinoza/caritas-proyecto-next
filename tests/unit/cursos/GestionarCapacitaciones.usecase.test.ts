...
describe('ListarInscripcionesUseCase', () => {
  it('[positivo] delega al repositorio', async () => {
    const inscripciones = [{ idInscripcion: 'i1', participante: 'Juan', estadoInscripcion: 'ACTIVO', ultimaNota: null, resultado: null, certificado: false }]
...
describe('RegistrarEvaluacionUseCase', () => {
  ...
  it('[negativo] lanza ValidationError con nota fuera del rango 0-20', async () => {
    const repo = makeRepo()
    await expect(
      new RegistrarEvaluacionUseCase(repo).execute('inscripcion-1', 21)
    ).rejects.toThrow(ValidationError)
  })
  ...
  it('[negativo] lanza ValidationError con nota negativa', async () => {
    const repo = makeRepo()
    await expect(
      new RegistrarEvaluacionUseCase(repo).execute('inscripcion-1', -1)
    ).rejects.toThrow(ValidationError)
  })
  ...
  it('[negativo] lanza NotFoundError cuando la inscripción no existe', async () => {
    ...
    await expect(
      new RegistrarEvaluacionUseCase(repo).execute('no-existe', 15)
    ).rejects.toThrow(NotFoundError)
  })
})
...
describe('CertificarUseCase', () => {
  ...
  it('[negativo] lanza BusinessRuleError si no tiene evaluación aprobada', async () => {
    ...
    await expect(
      new CertificarUseCase(repo).execute('inscripcion-1')
    ).rejects.toThrow(BusinessRuleError)
  })
  ...
  it('[negativo] lanza NotFoundError cuando la inscripción no existe', async () => {
    const repo = makeRepo({ existsInscripcionId: vi.fn().mockResolvedValue(false) })
    await expect(
      new CertificarUseCase(repo).execute('no-existe')
    ).rejects.toThrow(NotFoundError)
  })
})
