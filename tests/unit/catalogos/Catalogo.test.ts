...
describe('Catalogo.toggle', () => {
  ...
  it('[positivo] cambia de INACTIVO a ACTIVO', () => {
    const c = Catalogo.desdePersistencia({ id: 'cat-5', nombreCatalogo: 'Gravedad', estado: 'INACTIVO' })
    c.toggle()
    expect(c.snapshot.estado).toBe('ACTIVO')
  })
})
...
describe('CatalogoDetalle.crear', () => {
  it('[positivo] crea el detalle con estado ACTIVO', () => {
    const d = CatalogoDetalle.crear({ id: 'det-1', idCatalogoGRD: 'cat-1', codigo: 'SIS', valor: 'Sismo' })
  ...
  it('[positivo] trimea codigo y valor', () => {
    const d = CatalogoDetalle.crear({ id: 'det-2', idCatalogoGRD: 'cat-1', codigo: '  INC  ', valor: '  Incendio  ' })
  ...
  it('[negativo] lanza ValidationError cuando codigo está vacío', () => {
    expect(() => CatalogoDetalle.crear({ id: 'det-3', idCatalogoGRD: 'cat-1', codigo: '', valor: 'Sismo' })).toThrow(ValidationError)
  })

  it('[negativo] lanza ValidationError cuando valor está vacío', () => {
    expect(() => CatalogoDetalle.crear({ id: 'det-4', idCatalogoGRD: 'cat-1', codigo: 'SIS', valor: '' })).toThrow(ValidationError)
  })
})

describe('CatalogoDetalle.editar', () => {
  it('[positivo] actualiza el valor', () => {
    const d = CatalogoDetalle.crear({ id: 'det-5', idCatalogoGRD: 'cat-1', codigo: 'SIS', valor: 'Sismo' })
  ...
  it('[negativo] lanza ValidationError cuando el nuevo valor está vacío', () => {
    const d = CatalogoDetalle.crear({ id: 'det-6', idCatalogoGRD: 'cat-1', codigo: 'SIS', valor: 'Sismo' })
...
describe('CatalogoDetalle.toggle', () => {
  it('[positivo] cambia de ACTIVO a INACTIVO', () => {
    const d = CatalogoDetalle.crear({ id: 'det-7', idCatalogoGRD: 'cat-1', codigo: 'SIS', valor: 'Sismo' })
    d.toggle()
    expect(d.snapshot.estado).toBe('INACTIVO')
  })

  it('[positivo] cambia de INACTIVO a ACTIVO', () => {
    const d = CatalogoDetalle.desdePersistencia({ id: 'det-8', idCatalogoGRD: 'cat-1', codigo: 'SIS', valor: 'Sismo', estado: 'INACTIVO' })
    d.toggle()
    expect(d.snapshot.estado).toBe('ACTIVO')
  })
})
