...
async function estado(id: string): Promise<string> {
  const i = await prisma.incidencia.findUnique({ where: { idIncidencia: id }, select: { estadoActual: true } })
...
async function check(label: string, id: string, esperado: string) {
  ...
  if (bien) ok++
  console.log(`${bien ? '✅' : '❌'}  ${label}: estado = ${e}${bien ? '' : ` (esperaba ${esperado})`}`)
}
...
async function main() {
  ...
  const id = await uc.registrar.execute({
    reportaDni: '47823456', reportaNombre: 'Juan Pérez', reportaTel: '987654321', reportaRol: 'Vecino',
    fechaReporte: '2026-05-30', fechaSuceso: '2026-05-29', horaSuceso: '14:30',
    categoria: 'Incendio', pais: 'Perú', region: 'Lima', distrito: 'San Juan de Lurigancho',
    parroquia: 'Parroquia San Juan Bautista', direccion: 'Av. Los Jardines 456', referencia: 'Frente al mercado',
    descripcion: 'Incendio en edificio multifamiliar', causa: 'Cortocircuito',
    familias: [{ id: 'f1', nombre: 'Familia Pérez' }],
    personas: [{ id: 'p1', tipoDoc: 'DNI', dni: '12345678', nombre: 'Ana', apellidoPaterno: 'Pérez', apellidoMaterno: 'Soto', edad: '34', genero: 'Femenino', celular: '999', parentesco: 'Madre', situacionActual: 'Gestante', familiaId: 'f1' }],
    necesidades: ['Refugio temporal', 'Alimentación'], necesidadOtra: '', necesidadesObs: 'Urgente',
  ...
  const brig = await prisma.brigadistaParroquial.findFirst({ select: { idBrigadistaParroquial: true } })
  ...
  await uc.registrarCampo.execute(id, {
    fechaVisita: '2026-05-29', responsable: 'Tester', descripcionEvento: 'Daño severo',
    nivelVulnerabilidad: 'Alto', necesidadesPrioritarias: ['Refugio'], recomendacion: 'Apoyo urgente',
    condHabitabilidad: { agua: false, electricidad: false, refugio: false, saludAmbiental: false, acceso: true },
  }, 'Tester')
  await check('Levantamiento campo', id, 'DATA RECOPILADA')

  // 4. Informe de evaluación
  await uc.generarInforme.execute(id, {
    analisisSituacion: 'Alta vulnerabilidad', hallazgosTexto: 'Sin techo', conclusiones: 'Intervenir',
    nivelUrgencia: 'Inmediata', tipoIntervencion: 'Donación en especie', recomendacionComite: 'Aprobar',
  }, 'Tester')
  ...
  await uc.registrarAtencion.execute(id, { tipoAyuda: 'Kit de víveres', descripcionAyuda: 'Entrega de víveres', lugarEntrega: 'Domicilio' })
  await check('Atención', id, 'ATENDIDO')

  // 7. Seguimiento
  await uc.agregarSeguimiento.execute(id, { situacion: 'Mejoró', descripcion: 'Familia reubicada' })
  ...
  console.log(`   Informes: ${informes} · Solicitudes: ${solicitudes} · Entregas: ${entregas} · Seguimientos: ${seguimientos} · Historial: ${historial} · Personas: ${personas}`)
