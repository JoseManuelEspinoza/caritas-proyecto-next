...
async function main() {
  ...
  await step("Crear catálogo + detalle + editar + toggle + listar", async () => {
    ...
    const d = await cat.agregarDetalle.execute({
      idCatalogoGRD: c.id,
      codigo: "CHK1",
      valor: "Valor 1",
    });
  ...
  await step("Crear kit + ingreso + entrega + historial + stock", async () => {
    ...
    kitId = k.id;
    const k1 = await kit.registrarMovimiento.execute(k.id, {
      tipo: "INGRESO",
      cantidad: 5,
      idUsuarioResponsableGRD: idUsuarioGRD,
    });
    assert(k1.stockActual === 15, `stock tras ingreso debería ser 15, es ${k1.stockActual}`);
    const k2 = await kit.registrarMovimiento.execute(k.id, {
      tipo: "ENTREGA",
      cantidad: 3,
      idUsuarioResponsableGRD: idUsuarioGRD,
      idParroquiaDestino: idParroquia,
    });
  ...
  await step("Kit: bloquear entrega sin stock (invariante)", async () => {
    let bloqueado = false;
    try {
      await kit.registrarMovimiento.execute(kitId, {
        tipo: "ENTREGA",
        cantidad: 9999,
        idUsuarioResponsableGRD: idUsuarioGRD,
      });
  ...
  await step("Crear plan + enviar + aprobar", async () => {
    const p = await plan.crear.execute({
      idParroquia,
      idUsuarioResponsableGRD: idUsuarioGRD,
      nombrePlan: `CHK Plan ${Date.now()}`,
      objetivos: "check",
    });
  ...
  await step("Crear curso + publicar + inscribir + evaluar (aprobado) + certificar", async () => {
    const c = await curso.crear.execute({
      idUsuarioResponsableGRD: idUsuarioGRD,
      nombreCurso: `CHK Curso ${Date.now()}`,
    });
    await curso.cambiarEstado.execute(c.id, "PUBLICAR");
    const insc = await curso.inscribir.execute(c.id, {
      nombres: "Test",
      apellidos: "Participante",
      tipoDocumento: "DNI",
      numeroDocumento: `${Date.now()}`.slice(-8),
    });
  ...
  await step("Capacitaciones: no certificar sin aprobar (regla)", async () => {
    const c = await curso.crear.execute({
      idUsuarioResponsableGRD: idUsuarioGRD,
      nombreCurso: `CHK Curso2 ${Date.now()}`,
    });
    await curso.cambiarEstado.execute(c.id, "PUBLICAR");
    const insc = await curso.inscribir.execute(c.id, {
      nombres: "Test2",
