...
async function ensureDemoMasivo(idUsuarioGRD: string) {
  ...
  // 2. Brigadistas
  const nombres = ['Ana', 'Carlos', 'Luis', 'Rosa', 'María', 'Pedro', 'Lucía', 'Jorge', 'Elena', 'Miguel', 'Carmen', 'Diego', 'Patricia', 'Sofía', 'Raúl', 'Verónica']
  const apellidos = ['Torres Quispe', 'Mendoza Silva', 'Ramírez Pérez', 'Quispe Mamani', 'Flores García', 'Salas Rojas', 'Vargas Huamán', 'Castro León']

  let nBrig = await prisma.brigadistaParroquial.count()
  ...
  const estados = ['ABIERTO', 'ASIGNADO', 'DATA_RECOPILADA', 'EN_EVALUACION', 'APROBADO', 'ATENDIDO', 'SEGUIMIENTO_ABIERTO', 'CERRADO', 'RECHAZADO']
  const distritos = ['Comas', 'Ate', 'San Juan de Lurigancho', 'Villa El Salvador', 'San Martín de Porres', 'El Agustino', 'Puente Piedra', 'Independencia']

  let nInc = await prisma.incidencia.count()
  ...
  let nGrupos = await prisma.grupoFamiliarAfectado.count()
  ...
  let nPersonas = await prisma.personaAfectada.count()
  for (let i = nPersonas; i < TARGETS.personasAfectadas; i++) {
    await prisma.personaAfectada.create({
      data: {
        ...
        condicionSalud: i % 7 === 0 ? 'Requiere atención médica' : 'Sin condición crítica registrada',
  ...
  let nPlanes = await prisma.planTrabajoGRD.count()
  ...
  let nActividades = await prisma.actividadPreventiva.count()
  for (let i = nActividades; i < TARGETS.actividades; i++) {
    ...
    await prisma.actividadPreventiva.create({
      data: {
        ...
        codigoActividad,
        nombreActividad: i % 2 === 0 ? `Simulacro comunitario ${i + 1}` : `Charla de prevención ${i + 1}`,
        fechaProgramada: new Date(`2026-${pad((i % 6) + 6, 2)}-${pad((i % 20) + 1, 2)}`),
        fechaEjecucion: i % 3 === 0 ? new Date(`2026-${pad((i % 6) + 6, 2)}-${pad((i % 20) + 1, 2)}`) : null,
  ...
  let nMov = await prisma.movimientoKit.count()
  for (let i = nMov; i < TARGETS.movimientosKit; i++) {
    ...
    await prisma.movimientoKit.create({
      data: {
        ...
        idActividadPreventiva: i % 4 === 0 && actividades.length > 0 ? pick(actividades, i).idActividadPreventiva : null,
  ...
  let nCursos = await prisma.cursoCapacitacion.count()
  const cursosDemo = [
  ...
  let nIns = await prisma.inscripcionCurso.count()
  ...
  const refIds = [
    ...incidencias.map((x) => ({ tipo: tipoIncidencia, id: x.idIncidencia, modulo: 'INCIDENCIAS' })),
    ...actividades.map((x) => ({ tipo: tipoActividad, id: x.idActividadPreventiva, modulo: 'PREVENCION' })),
    ...kits.map((x) => ({ tipo: tipoKit, id: x.idKitEmergencia, modulo: 'LOGISTICA' })),
  ]

  let nEvid = await prisma.evidenciaGRD.count()
  ...
  let nObs = await prisma.observacionGRD.count()
  ...
  let nAud = await prisma.historialAuditoriaGRD.count()
  ...
  let nHist = await prisma.historialEstadoIncidencia.count()
  ...
  console.log(`     Parroquias: ${resumen[0]} · Brigadistas: ${resumen[1]} · Incidencias: ${resumen[2]}`)
  console.log(`     Familias: ${resumen[3]} · Personas afectadas: ${resumen[4]} · Kits: ${resumen[5]} · Movimientos: ${resumen[6]}`)
  console.log(`     Planes: ${resumen[7]} · Actividades: ${resumen[8]} · Cursos: ${resumen[9]} · Inscripciones: ${resumen[10]} · Progresos: ${resumen[11]}`)
  console.log(`     Evidencias: ${resumen[12]} · Observaciones: ${resumen[13]} · Auditorías: ${resumen[14]} · Historial estados: ${resumen[15]}`)
}
async function main() {
