...
export type DatosEditables = Partial<
  Pick<PlanProps, 'nombrePlan' | 'diagnosticoRiesgo' | 'objetivos' | 'actividadesGenerales' | 'fechaInicio' | 'fechaFin' | 'rutasEvacuacion' | 'zonasSeguras'>
>
...
export class PlanTrabajo {
  ...
  enviarRevision(): void { this.transicion('EN_REVISION') }
  ...
  private transicion(destino: EstadoAprobacion): void {
    if (!TRANSICIONES[this.props.estadoAprobacion].includes(destino)) {
      throw new BusinessRuleError(`Transición de aprobación no permitida: ${this.props.estadoAprobacion} → ${destino}.`)
    }
