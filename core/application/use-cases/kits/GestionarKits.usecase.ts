...
export class CrearKitUseCase {
  ...
  async execute(input: { tipoKit: string; descripcion?: string; stockInicial?: number; codigoAlmacen?: string; ubicacionAlmacen?: string }): Promise<KitOutput> {
...
export class RegistrarMovimientoKitUseCase {
  ...
  async execute(
    idKit: string,
    mov: { tipo: TipoMovimiento; cantidad: number; idUsuarioResponsableGRD: string; idParroquiaDestino?: string; motivoMovimiento?: string; observaciones?: string },
  ): Promise<KitOutput> {
