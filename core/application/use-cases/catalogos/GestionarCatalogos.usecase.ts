...
export interface CatalogoOutput { id: string; nombreCatalogo: string; descripcion: string | null; estado: string }
export interface DetalleOutput { id: string; idCatalogoGRD: string; codigo: string; valor: string; descripcion: string | null; orden: number | null; estado: string }

const catOut = (c: Catalogo): CatalogoOutput => {
  const s = c.snapshot
  return { id: s.id, nombreCatalogo: s.nombreCatalogo, descripcion: s.descripcion ?? null, estado: s.estado }
}
const detOut = (d: CatalogoDetalle): DetalleOutput => {
  const s = d.snapshot
  return { id: s.id, idCatalogoGRD: s.idCatalogoGRD, codigo: s.codigo, valor: s.valor, descripcion: s.descripcion ?? null, orden: s.orden ?? null, estado: s.estado }
}
...
export class AgregarDetalleUseCase {
  constructor(private readonly repo: ICatalogoRepository) {}
  async execute(input: { idCatalogoGRD: string; codigo: string; valor: string; descripcion?: string; orden?: number }): Promise<DetalleOutput> {
