...
export class PrismaCatalogoRepository implements ICatalogoRepository {
  ...
  async existsNombreCatalogo(nombre: string): Promise<boolean> {
    const row = await prisma.catalogoGRD.findUnique({ where: { nombreCatalogo: nombre }, select: { idCatalogoGRD: true } })
    return row !== null
  }
  ...
  async findDetallesByCatalogo(idCatalogo: string): Promise<CatalogoDetalle[]> {
    const rows = await prisma.catalogoDetalleGRD.findMany({ where: { idCatalogoGRD: idCatalogo }, orderBy: [{ orden: 'asc' }, { valor: 'asc' }] })
