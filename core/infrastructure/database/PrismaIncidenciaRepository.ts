...
export class PrismaIncidenciaRepository implements IIncidenciaRepository {
  async nextCodigo(): Promise<string> {
    ...
    const num = last?.codigoCaso ? parseInt(last.codigoCaso.replace(`GRD-${year}-`, '') || '0', 10) : 0
  ...
  async findById(id: string): Promise<Incidencia | null> {
    const row = await prisma.incidencia.findUnique({ where: { idIncidencia: id }, select: SELECT_ESTADO })
    return row ? IncidenciaMapper.toDomain(row) : null
  }

  async actualizarDatos(id: string, data: CreateIncidenteData): Promise<void> {
    const inc = await prisma.incidencia.findUnique({ where: { idIncidencia: id }, select: { idAviso: true, estadoActual: true } })
    if (!inc) return
    ...
    if (inc.idAviso) {
      ...
      await prisma.incidencia.update({ where: { idIncidencia: id }, data: { idAviso: aviso.idAviso } })
    }

    // Reemplazar grupos y personas
    const grupos = await prisma.grupoFamiliarAfectado.findMany({ where: { idIncidencia: id }, select: { idGrupoFamiliar: true } })
  ...
  async guardarInforme(idIncidencia: string, informe: { titulo: string; tipo: string; resumen: string; contenido: string; estado: string }): Promise<void> {
  ...
  async upsertSolicitudEnEvaluacion(idIncidencia: string, data: { motivo: string; descripcion: string; tipoAyuda: string }): Promise<void> {
  ...
  async resolverSolicitud(idIncidencia: string, decision: 'APROBADA' | 'EN_EVALUACION' | 'RECHAZADA', observaciones?: string): Promise<void> {
  ...
  async registrarEntrega(idIncidencia: string, data: { tipoAyuda: string; descripcionAyuda: string; lugarEntrega: string; observaciones?: string }): Promise<void> {
  ...
  async agregarSeguimiento(idIncidencia: string, data: { situacion: string; descripcion: string; necesidadesPendientes?: string; recomendaciones?: string }): Promise<void> {
  ...
  private async buscarParroquia(nombre: string): Promise<string | null> {
    ...
    const rec = await prisma.parroquia.findFirst({ where: { nombre }, select: { idParroquia: true } })
  ...
  private alias(data: CreateIncidenteData): string {
    ...
    return parrShort ? `${data.categoria}-${data.distrito}-${parrShort}` : `${data.categoria}-${data.distrito}`
  }
  ...
  private async recrearGruposYPersonas(idIncidencia: string, data: CreateIncidenteData): Promise<void> {
    ...
    for (const familia of data.familias) {
      const grupo = await prisma.grupoFamiliarAfectado.create({
        data: { idIncidencia, nombreReferencia: familia.nombre, observaciones: familia.observaciones || null },
      })
    ...
    for (const persona of data.personas) {
      ...
      await prisma.personaAfectada.create({
        data: {
          ...
          fechaNacimiento: persona.edad ? new Date(new Date().getFullYear() - parseInt(persona.edad), 0, 1) : null,
        },
      })
    }
  }
