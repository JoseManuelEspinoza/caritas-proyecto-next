  ...
  async crear(codigo: string, data: CreateIncidenteData): Promise<string> {
    ...
    const aviso = await prisma.avisoEmergencia.create({
      data: {
        ...
        dniInformante: data.reportaDni.replace(/\D/g, "") || null,
        rolInformante: data.reportaRol || null,
    ...
    const incidencia = await prisma.incidencia.create({
      data: {
        ...
        fechaSuceso: data.fechaSuceso ? new Date(data.fechaSuceso) : null,
        horaSuceso: data.horaSuceso || null,
        ...(idParroquiaDb ? { idParroquia: idParroquiaDb } : {}),
        ...coords(data),
      },
    });
