...
function normalizar(json: Record<string, unknown>): DatosDni | null {
  ...
  const apellidoPaterno = str('apellidoPaterno', 'apellido_paterno', 'ape_paterno', 'first_last_name', 'apepaterno')
  const apellidoMaterno = str('apellidoMaterno', 'apellido_materno', 'ape_materno', 'second_last_name', 'apematerno')
...
export async function consultarDni(dni: string): Promise<Resultado> {
  ...
  if (!urlTpl) {
    return { ok: false, error: 'Consulta DNI no configurada. Define RENIEC_API_URL en el servidor.' }
  }
  ...
  try {
    ...
    if (res.status === 401 || res.status === 403) return { ok: false, error: 'Token de consulta inválido o vencido.' }
