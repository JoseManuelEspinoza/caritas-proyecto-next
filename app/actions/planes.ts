...
export async function actualizarPlan(id: string, datos: DatosEditables) {
  ...
  await logGRDAction({
    ...
    entity: "Plan",
    entityId: id,
    entityName: datos.nombrePlan ?? id,
    module: "Planes",
  });
  revalidatePath(REVALIDATE);
}

export async function cambiarAprobacionPlan(
  id: string,
  accion: "ENVIAR" | "APROBAR" | "OBSERVAR",
