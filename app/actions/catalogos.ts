...
export async function crearCatalogo(nombreCatalogo: string, descripcion?: string) {
  ...
  await logGRDAction({ userId: session.userId, action: 'CREAR', entity: 'Catálogo', entityId: nombreCatalogo, entityName: nombreCatalogo, module: 'Catálogos' })
...
export async function agregarItemCatalogo(input: { idCatalogoGRD: string; codigo: string; valor: string; descripcion?: string; orden?: number }) {
  ...
  await logGRDAction({ userId: session.userId, action: 'CREAR', entity: 'Item Catálogo', entityId: input.idCatalogoGRD, entityName: input.valor, module: 'Catálogos' })
  revalidatePath(REVALIDATE)
}
...
export async function editarItemCatalogo(id: string, valor: string, descripcion?: string) {
  ...
  await logGRDAction({ userId: session.userId, action: 'EDITAR', entity: 'Item Catálogo', entityId: id, entityName: valor, module: 'Catálogos', field: 'Valor', newValue: valor })
  revalidatePath(REVALIDATE)
}
...
export async function toggleItemCatalogo(id: string) {
  ...
  await logGRDAction({ userId: session.userId, action: 'EDITAR', entity: 'Item Catálogo', entityId: id, entityName: id, module: 'Catálogos', field: 'Estado', newValue: 'toggled' })
  revalidatePath(REVALIDATE)
}
