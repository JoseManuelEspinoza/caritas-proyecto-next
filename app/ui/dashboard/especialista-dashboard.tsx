...
import {
  AlertTriangle, Users, ShieldCheck, Clock, CheckCircle,
  ArrowRight, MapPin, Flame, Waves, Mountain, Zap, TrendingDown,
  CircleDot, Package,
} from 'lucide-react'
...
export function EspecialistaDashboard({
  userName, incidentesActivos, familias, personas,
  brigDisp, totalBrigActivos, pipelineCounts,
  urgentes, brigadistasDisponibles, incidentesRecientes, simulacrosActivos,
  ...
  return (
    <div className="p-4 md:p-6 space-y-5 max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <StatCard label="Acciones urgentes"        value={urgentes.length}   icon={Clock}         color="bg-[#FF823C]" sub={urgentes.length > 0 ? 'Requieren tu atención' : 'Sin pendientes'} />
        <StatCard label="Familias afectadas"       value={familias}          icon={Users}         color="bg-[#9155A8]" sub={`${personas} personas`} />
        <StatCard label="Brigadistas disponibles"  value={brigDisp}          icon={ShieldCheck}   color="bg-[#00C8B4]" to="/brigadistas" />
      </div>
      ...
      <div className="bg-white border border-[#DDDDDD] rounded-xl p-4">
        <div className="flex items-center justify-between mb-3">
          ...
          <Link href="/grd" className="text-xs text-[#009850] hover:underline flex items-center gap-1">Ver todo <ArrowRight className="w-3 h-3" /></Link>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="bg-white border border-[#DDDDDD] rounded-xl overflow-hidden">
          ...
          <div className="p-3 space-y-2 max-h-64 overflow-y-auto">
            {urgentes.length === 0 ? (
              ...
              <Link key={inc.id} href={`/grd/${inc.id}`}
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-semibold truncate">{inc.tituloIncidencia ?? 'Sin título'}</p>
                  <p className="text-[10px] opacity-70">{inc.codigoCaso ?? '—'}</p>
                </div>
                <span className="text-[10px] font-bold px-2 py-0.5 bg-white/40 rounded-full flex-shrink-0">{label}</span>
              </Link>
            ))}
          </div>
        </div>
        ...
        <div className="bg-white border border-[#DDDDDD] rounded-xl overflow-hidden">
          <div className="px-4 py-3 border-b border-[#DDDDDD] flex items-center gap-2">
            ...
            <span className="ml-auto text-xs text-gray-500">{brigDisp}/{totalBrigActivos} activos</span>
          </div>
          <div className="divide-y divide-[#DDDDDD] max-h-64 overflow-y-auto">
            ...
            {brigadistasDisponibles.length > 5 && (
              <Link href="/brigadistas" className="flex items-center justify-center gap-1 py-2 text-xs text-[#009850] hover:underline">
                Ver los {brigadistasDisponibles.length} disponibles <ArrowRight className="w-3 h-3" />
              </Link>
            )}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="bg-white border border-[#DDDDDD] rounded-xl overflow-hidden">
          <div className="px-4 py-3 border-b border-[#DDDDDD] flex items-center justify-between">
            ...
            <Link href="/simulacros" className="text-xs text-[#009850] hover:underline">Ver todos</Link>
          </div>
          <div className="p-3 space-y-2">
            {simulacrosActivos.length === 0 ? (
              ...
              <div key={s.id} className="flex items-start gap-2.5 p-2.5 rounded-lg bg-gray-50">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-[11px] font-semibold text-gray-800 truncate">{s.nombreActividad}</p>
                  ...
                  <span className={`inline-block mt-0.5 text-[9px] font-semibold px-1.5 py-0.5 rounded-full ${
                    s.estadoActividad === 'PROGRAMADA'   ? 'bg-blue-50 text-blue-700' :
                    s.estadoActividad === 'EN_EJECUCION' ? 'bg-indigo-50 text-indigo-700' :
                    'bg-amber-50 text-amber-700'
                  }`}>{s.estadoActividad}</span>
                </div>
              </div>
            ))}
            <Link href="/simulacros" className="flex items-center justify-center gap-1 py-1.5 text-xs text-[#009850] hover:underline w-full">
