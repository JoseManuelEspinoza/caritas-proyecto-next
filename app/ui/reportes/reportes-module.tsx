  ...
  return (
    <div className="p-4 md:p-6 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mb-6">
        <Card
          icon={<AlertTriangle className="w-5 h-5 text-[var(--caritas-green)]" />}
          label="Incidencias"
          value={totales.incidencias}
        />
        <Card
          icon={<Users className="w-5 h-5 text-[var(--caritas-green)]" />}
          label="Brigadistas"
          value={totales.brigadistas}
          sub={`${totales.brigadistasActivos} activos`}
        />
        <Card
          icon={<Package className="w-5 h-5 text-[var(--caritas-green)]" />}
          label="Kits"
          value={totales.kits}
          sub={`stock total ${totales.stockTotal}`}
        />
        <Card
          icon={<GraduationCap className="w-5 h-5 text-[var(--caritas-green)]" />}
          label="Cursos"
          value={totales.cursos}
        />
        <Card
          icon={<ClipboardList className="w-5 h-5 text-[var(--caritas-green)]" />}
          label="Planes GRD"
          value={totales.planes}
        />
        <Card
          icon={<ClipboardList className="w-5 h-5 text-[var(--caritas-green)]" />}
          label="Actividades preventivas"
          value={totales.actividades}
        />
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        <div className="bg-white border border-[var(--caritas-border)] rounded-xl p-5">
          <h2 className="text-sm font-semibold text-[var(--caritas-text)] mb-4">
