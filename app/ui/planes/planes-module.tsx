...
export function PlanesModule({ planes, parroquias }: { planes: Plan[]; parroquias: Parroquia[] }) {
  ...
  const submit = () => {
    ...
    run(async () => {
      ...
      if (!res?.message) { setShowForm(false); setForm({ ...form, nombrePlan: '', objetivos: '', diagnosticoRiesgo: '' }) }
      return res
    }, 'Plan creado.')
  }
      ...
      {showForm && (
        <div className="bg-white border border-[var(--caritas-border)] rounded-xl p-5 mb-6 grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          <label className="block">
            ...
            <input value={form.nombrePlan} onChange={(e) => setForm({ ...form, nombrePlan: e.target.value })} className="mt-1 w-full px-3 py-2 border border-[var(--caritas-border)] rounded text-sm" />
          </label>
          <label className="block md:col-span-2">
            <span className="text-xs text-gray-600">Objetivos</span>
            <textarea value={form.objetivos} onChange={(e) => setForm({ ...form, objetivos: e.target.value })} rows={2} className="mt-1 w-full px-3 py-2 border border-[var(--caritas-border)] rounded text-sm" />
          </label>
          <label className="block md:col-span-2">
            <span className="text-xs text-gray-600">Diagnóstico de riesgo</span>
            <textarea value={form.diagnosticoRiesgo} onChange={(e) => setForm({ ...form, diagnosticoRiesgo: e.target.value })} rows={2} className="mt-1 w-full px-3 py-2 border border-[var(--caritas-border)] rounded text-sm" />
          </label>
          <label className="block">
            <span className="text-xs text-gray-600">Fecha inicio</span>
            <input type="date" value={form.fechaInicio} onChange={(e) => setForm({ ...form, fechaInicio: e.target.value })} className="mt-1 w-full px-3 py-2 border border-[var(--caritas-border)] rounded text-sm" />
          </label>
          <label className="block">
            <span className="text-xs text-gray-600">Fecha fin</span>
            <input type="date" value={form.fechaFin} onChange={(e) => setForm({ ...form, fechaFin: e.target.value })} className="mt-1 w-full px-3 py-2 border border-[var(--caritas-border)] rounded text-sm" />
          </label>
            ...
            <div className="flex flex-wrap items-start justify-between gap-3 mb-3">
              <div>
                ...
                <div className="text-sm text-gray-600 flex flex-wrap items-center gap-3 mt-1">
                  ...
                  <span className="flex items-center gap-1"><Calendar className="w-3 h-3" /> {p.fechaInicio?.slice(0, 10) ?? '—'} → {p.fechaFin?.slice(0, 10) ?? '—'}</span>
                </div>
              </div>
              <span className={`text-xs px-2 py-1 rounded ${ESTADO_BADGE[p.estadoAprobacion] ?? 'bg-gray-100 text-gray-700'}`}>{p.estadoAprobacion.replace('_', ' ')}</span>
            </div>
            {p.objetivos && <p className="text-sm text-gray-700 mb-3">{p.objetivos}</p>}
            {p.observaciones && p.estadoAprobacion === 'OBSERVADO' && (
              <p className="text-sm text-orange-700 bg-orange-50 rounded p-2 mb-3 flex items-start gap-2"><AlertCircle className="w-4 h-4 mt-0.5" /> {p.observaciones}</p>
            )}

            <div className="flex flex-wrap gap-2">
              {(p.estadoAprobacion === 'BORRADOR' || p.estadoAprobacion === 'OBSERVADO') && (
                <button onClick={() => run(() => cambiarAprobacionPlan(p.id, 'ENVIAR'), 'Plan enviado a revisión.')} disabled={pending} className="flex items-center gap-1.5 px-3 py-1.5 text-sm bg-blue-600 text-white rounded disabled:opacity-50">
                  <Send className="w-3.5 h-3.5" /> Enviar a revisión
                </button>
              )}
              {p.estadoAprobacion === 'EN_REVISION' && (
                <>
                  <button onClick={() => run(() => cambiarAprobacionPlan(p.id, 'APROBAR'), 'Plan aprobado.')} disabled={pending} className="flex items-center gap-1.5 px-3 py-1.5 text-sm bg-[var(--caritas-green)] text-white rounded disabled:opacity-50">
                    <CheckCircle2 className="w-3.5 h-3.5" /> Aprobar
                  </button>
                  <button
                    onClick={() => {
                      const obs = window.prompt('Observaciones:')
                      if (obs) run(() => cambiarAprobacionPlan(p.id, 'OBSERVAR', obs), 'Plan observado.')
                    }}
                    disabled={pending}
