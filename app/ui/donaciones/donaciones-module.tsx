...
export function DonacionesModule({ casos, canEvaluate }: { casos: Caso[]; canEvaluate: boolean }) {
  ...
  const decidir = (accion: 'APROBAR' | 'OBSERVAR' | 'RECHAZAR') => {
    if (!current) return
    if (!notes.trim()) { toast.error('Escribe la justificación de la decisión.'); return }
    const fn = accion === 'APROBAR' ? () => aprobarCaso(current.id, notes)
      : accion === 'OBSERVAR' ? () => observarCaso(current.id, notes)
      : () => rechazarCaso(current.id, notes)
    startTransition(async () => {
      const res = await fn()
      if (res?.message && /no se pudo|no permitida|obligatori/i.test(res.message)) { toast.error(res.message); return }
  ...
  return (
    <div className="p-4 md:p-6 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        <Stat n={queue.length} label="Pendientes de evaluación" cls="bg-purple-50 border-purple-200 text-purple-900" />
        <Stat n={casos.filter((c) => c.estado === 'APROBADO' || c.estado === 'ATENDIDO').length} label="Aprobadas" cls="bg-green-50 border-green-200 text-green-900" />
        <Stat n={casos.filter((c) => c.estado === 'SEGUIMIENTO ABIERTO').length} label="En seguimiento" cls="bg-teal-50 border-teal-200 text-teal-900" />
        <Stat n={casos.filter((c) => c.estado === 'RECHAZADO').length} label="Rechazadas" cls="bg-red-50 border-red-200 text-red-900" />
      </div>
        ...
        <div className="lg:col-span-3">
          {!current ? (
            <div className="bg-white border border-[var(--caritas-border)] rounded-xl p-12 text-center h-full flex flex-col items-center justify-center">
              ...
              <p className="text-gray-500 text-sm font-medium">Selecciona un caso para ver el detalle</p>
            </div>
          ) : (
            <div className="bg-white border border-[var(--caritas-border)] rounded-xl overflow-hidden">
              <div className="p-4 border-b border-gray-100 flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className="text-base font-bold text-gray-900 leading-tight">{current.titulo}</p>
                  <p className="text-xs text-gray-500 mt-0.5">{[current.direccion, current.parroquia].filter(Boolean).join(' · ')}</p>
                </div>
                <span className={`flex-shrink-0 px-3 py-1 text-xs rounded-full font-semibold ${STATUS_COLOR[current.estado] ?? 'bg-gray-100'}`}>{current.estado}</span>
              </div>
              ...
              <div className="p-4 space-y-3">
                ...
                {current.informe && (
                  <div className="bg-purple-50 border border-purple-200 rounded-xl p-4 space-y-3">
                    <p className="text-xs font-bold text-purple-800 uppercase tracking-wider">Informe del Especialista GRD</p>
                    ...
                    {current.informe.analisisSituacion && <Block label="Análisis de la situación" value={current.informe.analisisSituacion} />}
                    {current.informe.hallazgosTexto && <Block label="Hallazgos" value={current.informe.hallazgosTexto} />}
                    {current.informe.recomendacionComite && <Block label="Recomendación al Comité" value={`"${current.informe.recomendacionComite}"`} italic />}
                  </div>
                )}
                ...
                {canEvaluate && PENDIENTES.includes(current.estado) ? (
                  <div className="border-t border-gray-100 pt-4 space-y-3">
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      <button onClick={() => decidir('APROBAR')} disabled={pending} className="flex flex-col items-center gap-1 py-2.5 bg-green-600 hover:bg-green-700 text-white font-bold text-xs rounded-lg disabled:opacity-50"><CheckCircle className="w-4 h-4" /> Aprobar</button>
                      <button onClick={() => decidir('OBSERVAR')} disabled={pending} className="flex flex-col items-center gap-1 py-2.5 bg-yellow-500 hover:bg-yellow-600 text-white font-bold text-xs rounded-lg disabled:opacity-50"><AlertCircle className="w-4 h-4" /> Observar</button>
                      <button onClick={() => decidir('RECHAZAR')} disabled={pending} className="flex flex-col items-center gap-1 py-2.5 bg-red-500 hover:bg-red-600 text-white font-bold text-xs rounded-lg disabled:opacity-50"><XCircle className="w-4 h-4" /> Rechazar</button>
                    </div>
                  </div>
                ) : (
                  <Link href={`/grd/${current.id}`} className="mt-2 flex items-center gap-1.5 text-xs text-[var(--caritas-green)] hover:underline">
...
function CasoRow({ c, selected, onClick, compact }: { c: Caso; selected: boolean; onClick: () => void; compact?: boolean }) {
  return (
    <button onClick={onClick} className={`w-full text-left px-4 ${compact ? 'py-2.5' : 'py-3'} hover:bg-purple-50 transition-colors ${selected ? 'bg-purple-50 border-r-4 border-r-purple-600' : ''}`}>
      <div className="flex items-center justify-between gap-2">
        <div className="min-w-0">
          <p className="text-xs font-mono text-gray-400">{c.codigo}</p>
          <p className={`${compact ? 'text-xs font-medium' : 'text-sm font-semibold'} text-gray-900 leading-tight truncate`}>{c.titulo}</p>
        </div>
        <span className={`flex-shrink-0 px-2 py-0.5 text-[10px] rounded-full font-semibold ${STATUS_COLOR[c.estado] ?? 'bg-gray-100'}`}>{c.estado}</span>
      </div>
    </button>
  )
}
