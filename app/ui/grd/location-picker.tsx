...
export function LocationPicker({ lat, lng, onChange, onAddressResolved }: Props) {
  ...
  return (
    <div className="space-y-2">
      ...
      {expanded && (
        <div className="fixed inset-0 z-[1000] bg-black/60 flex items-center justify-center p-3 md:p-6" onClick={() => setExpanded(false)}>
          <div className="bg-white rounded-xl w-full max-w-5xl h-[88vh] flex flex-col shadow-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200">
              <div className="flex items-center gap-2">
                <MapPin className="w-4 h-4 text-[#009850]" />
                <h3 className="text-sm font-bold text-gray-800">Seleccionar ubicación en el mapa</h3>
                {geocoding && <span className="flex items-center gap-1 text-xs text-[#009850]"><Loader2 className="w-3 h-3 animate-spin" /> Buscando dirección…</span>}
              </div>
              <button type="button" onClick={() => setExpanded(false)} className="p-1.5 text-gray-500 hover:bg-gray-100 rounded-lg" title="Cerrar">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="flex items-center justify-between gap-3 px-4 py-3 border-t border-gray-200 bg-gray-50">
              <p className="text-xs text-gray-600">
                {hasPoint
                  ? <>Seleccionado: <span className="font-mono font-semibold">{lat}, {lng}</span></>
                  : 'Haz click en el mapa para marcar la ubicación.'}
              </p>
              <button type="button" onClick={() => setExpanded(false)} className="px-4 py-2 bg-[#009850] text-white rounded-lg text-sm font-semibold hover:opacity-90">
                Listo
              </button>
            </div>
          </div>
