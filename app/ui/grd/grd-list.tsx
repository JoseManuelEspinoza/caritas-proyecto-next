...
import {
  ...
  Clock, CheckCircle, XCircle, FileCheck, ClipboardList,
  HandHeart, Flame, Waves, Mountain, Zap, TrendingDown,
} from 'lucide-react'
...
type StatusCfg = { label: string; badge: string; card: string; ring: string; icon: any; iconBg: string }

const STATUS: Record<string, StatusCfg> = {
  'ABIERTO':             { label: 'Abierto',          badge: 'bg-yellow-100 text-yellow-800', card: 'from-yellow-50 to-yellow-100 border-yellow-200', ring: 'ring-yellow-500', icon: Clock,       iconBg: 'bg-yellow-600' },
  'ASIGNADO':            { label: 'Asignado',          badge: 'bg-blue-100 text-blue-800',    card: 'from-blue-50 to-blue-100 border-blue-200',       ring: 'ring-blue-500',   icon: CheckCircle, iconBg: 'bg-blue-600' },
  'DATA RECOPILADA':     { label: 'Data Recopilada',   badge: 'bg-orange-100 text-orange-800',card: 'from-orange-50 to-orange-100 border-orange-200',  ring: 'ring-orange-500', icon: ClipboardList,iconBg: 'bg-orange-500' },
  'EN EVALUACION':       { label: 'En Evaluación',     badge: 'bg-purple-100 text-purple-800',card: 'from-purple-50 to-purple-100 border-purple-200',  ring: 'ring-purple-500', icon: FileCheck,   iconBg: 'bg-purple-600' },
  'OBSERVADO':           { label: 'Observado',         badge: 'bg-amber-100 text-amber-800',  card: 'from-amber-50 to-amber-100 border-amber-200',    ring: 'ring-amber-500',  icon: FileCheck,   iconBg: 'bg-amber-500' },
  'APROBADO':            { label: 'Aprobado',          badge: 'bg-green-100 text-green-800',  card: 'from-green-50 to-green-100 border-green-200',    ring: 'ring-green-500',  icon: CheckCircle, iconBg: 'bg-green-600' },
  'ATENDIDO':            { label: 'Atendido',          badge: 'bg-cyan-100 text-cyan-800',    card: 'from-cyan-50 to-cyan-100 border-cyan-200',       ring: 'ring-cyan-500',   icon: HandHeart,   iconBg: 'bg-cyan-600' },
  'SEGUIMIENTO ABIERTO': { label: 'Seguimiento',       badge: 'bg-teal-100 text-teal-800',    card: 'from-teal-50 to-teal-100 border-teal-200',       ring: 'ring-teal-500',   icon: FileCheck,   iconBg: 'bg-teal-600' },
  'CERRADO':             { label: 'Cerrado',           badge: 'bg-gray-100 text-gray-700',    card: 'from-gray-50 to-gray-100 border-gray-200',       ring: 'ring-gray-500',   icon: XCircle,     iconBg: 'bg-gray-600' },
  'RECHAZADO':           { label: 'Rechazado',         badge: 'bg-red-100 text-red-700',      card: 'from-red-50 to-red-100 border-red-200',          ring: 'ring-red-500',    icon: XCircle,     iconBg: 'bg-red-600' },
}
  ...
  return (
    <div className="p-4 md:p-6 space-y-4 md:space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl md:text-2xl font-semibold text-gray-900">Gestión de Riesgo de Desastres</h1>
          <p className="text-sm text-gray-500 mt-0.5">Seguimiento y gestión de incidentes — Cáritas Lima</p>
        </div>
        {canCreate && (
          <Link
            href="/grd/nuevo"
            ...
            >
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-semibold text-gray-900 truncate">{item.tituloIncidencia ?? 'Sin título'}</p>
                  <p className="text-xs text-gray-500">{item.codigoCaso} · {new Date(item.fechaRegistro).toLocaleDateString('es-PE')}</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-4 text-xs text-gray-500">
                {item.direccionEvento && (
                  <span className="flex items-center gap-1"><MapPin className="w-3 h-3" />{item.direccionEvento}</span>
                )}
                {item.totalFamilias > 0 && (
                  <span className="flex items-center gap-1"><Users className="w-3 h-3" />{item.totalFamilias} familias</span>
                )}
              </div>
            </Link>
          )
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-[#F5F5F5] border-b border-[#DDDDDD]">
                <tr>
                  {['Código', 'Incidente', 'Responsable', 'Categoría', 'Ubicación', 'Estado', 'Fecha'].map((h) => (
                    <th key={h} className="px-5 py-3.5 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider whitespace-nowrap">{h}</th>
                  ))}
                </tr>
              </thead>
                  ...
                  return (
                    ...
                    >
                      <td className="px-5 py-4 whitespace-nowrap">
                        <span className="font-mono text-sm text-gray-800">{item.codigoCaso ?? '—'}</span>
                      </td>
                      <td className="px-5 py-4 max-w-xs">
                        <p className="font-medium text-gray-900 truncate">{item.tituloIncidencia ?? 'Sin título'}</p>
                        <div className="flex items-center gap-3 mt-0.5 text-xs text-gray-400">
                          {item.direccionEvento && (
                            <span className="flex items-center gap-1"><MapPin className="w-3 h-3" />{item.direccionEvento}</span>
                          )}
                          {item.totalPersonas > 0 && (
                            <span className="flex items-center gap-1"><Users className="w-3 h-3" />{item.totalPersonas} personas</span>
                          )}
                        </div>
                      </td>
                      <td className="px-5 py-4 whitespace-nowrap">
                      ...
                      <td className="px-5 py-4 whitespace-nowrap">
                        <span className="text-sm text-gray-700">{item.parroquia ?? item.direccionEvento ?? '—'}</span>
                      </td>
                      <td className="px-5 py-4 whitespace-nowrap">
                        <span className={`px-2.5 py-0.5 rounded-full text-xs font-semibold ${cfg.badge}`}>{cfg.label}</span>
                      </td>
