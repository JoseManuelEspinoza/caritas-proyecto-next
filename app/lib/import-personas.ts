...
export function mapGenero(raw: string): string {
  const v = norm(raw);
  if (!v) return "";
  if (v === "m" || v.startsWith("masc") || v.startsWith("hombre") || v.startsWith("varon"))
    return "Masculino";
...
export function mapSituacion(raw: string): string {
  ...
  if (
    v.includes("adulto mayor") ||
    v.includes("anciano") ||
    v.includes("tercera edad") ||
    v.includes("adulto_mayor")
  )
    return "Adulto mayor";
...
export function parsePersonasFlexible(texto: string): ImportResult {
  ...
  const flush = () => {
    if (Object.keys(raw).length > 0) {
      personas.push(buildPersona(raw, personas.length + 1));
      raw = {};
    }
    ultimoOrden = -1;
  };
...
function buildPersona(
  raw: Partial<Record<keyof PersonaImportada, string>>,
  bloque: number
): PersonaImportada {
  return {
...
export function validarPersona(
  p: PersonaImportada
): Partial<Record<keyof PersonaImportada, string>> {
