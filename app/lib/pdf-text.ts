...
function construirFilas(items: { str: string; transform: number[]; width: number }[]): Fila[] {
  ...
  for (const it of items) {
    ...
    porY.get(y)!.push({ x: it.transform[4], text: it.str, w: it.width, fs: Math.abs(it.transform[0]) || 10 })
  }
  const filas: Fila[] = []
  for (const y of Array.from(porY.keys()).sort((a, b) => b - a)) {
    ...
    for (const it of its) {
      ...
      if (cur && it.x - prevEnd > charW * 1.6) { celdas.push({ x: curX, text: cur.trim() }); cur = '' }
      if (!cur) curX = it.x
      cur += (cur ? ' ' : '') + it.text
...
async function extraerCapaTexto(
  pdf: Awaited<ReturnType<typeof cargarPdf>>, onProgress?: ProgresoCb,
...
export function tablaDeFilas(filas: Fila[]): Record<string, string>[] {
  ...
  for (let i = headerIdx + 1; i < filas.length; i++) {
    ...
    for (const celda of fila.celdas) {
      let best = cols[0]
      let bestD = Infinity
      for (const col of cols) { const d = Math.abs(col.x - celda.x); if (d < bestD) { bestD = d; best = col } }
